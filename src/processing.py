...
def import_data():
    ...
    data.drop(labels=["state", "county", "community",
              "communityname"], axis=1, inplace=True)
    ...
    col_names_missing_vals = pd.read_csv(
        "communities_names_missing.csv", header=None)
    ...
    protected_cols = ["IsBlack", "racepctblack",
